...
// API function to update SEO entry using PATCH
const updatePageSeo = async ({ id, seoData }) => {
  const response = await fetch(
    `https://codingcloud.pythonanywhere.com/page-seo/${id}/`,
    {
      method: "PATCH",
...
export default function EditSeo() {
  ...
  useEffect(() => {
    if (seoEntry) {
      setFormData({
        page_name: seoEntry.page_name || "",
        meta_title: seoEntry.meta_title || "",
        meta_description: seoEntry.meta_description || "",
        meta_keywords: seoEntry.meta_keywords || "",
      });
    }
  }, [seoEntry]);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-3 text-gray-500">Loading SEO data...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl shadow-lg p-8 max-w-md w-full text-center">
          ...
          <h3 className="text-base font-bold text-gray-900 mb-2">
            Could not load SEO entry
          </h3>
          <p className="text-sm text-gray-500 mb-5">{fetchError.message}</p>
          <button
