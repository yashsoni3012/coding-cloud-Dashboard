...
export default function AddCourse() {
  ...
  const [formData, setFormData] = useState({
    name: "", slug: "", category: "", text: "", duration: "",
    lecture: "", students: "", level: "", language: "", certificate: "No",
    meta_title: "", meta_description: "", keywords: "",
    image: null, banner_img: null, pdf_file: null, icon: null,
  });
  ...
  const UploadBox = ({ preview, onRemove, inputId, inputName, accept, label, hint, isSmall }) => (
    <div>
      ...
      <div style={{
        ...
        {preview ? (
          <div style={{ position: "relative", display: "inline-block" }}>
            ...
            <button type="button" onClick={onRemove}
              style={{ position: "absolute", top: -8, right: -8, width: 26, height: 26, borderRadius: "50%", background: "#ef4444", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 2px 6px rgba(239,68,68,0.4)" }}>
              <X size={12} color="#fff" />
            </button>
          </div>
        ) : (
        ...
        <input type="file" name={inputName} accept={accept} onChange={handleFileChange} className="hidden" id={inputId} style={{ display: "none" }} />
      </div>
    </div>
  );
  ...
  const PdfBox = () => (
    <div>
      ...
      <div style={{
        ...
        {pdfName ? (
          <div style={{ position: "relative", display: "inline-block" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", background: "#fff", borderRadius: 10, boxShadow: "0 1px 6px rgba(0,0,0,0.08)" }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 500, color: "#374151", maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{pdfName}</span>
            </div>
            <button type="button" onClick={() => removeFile("pdf_file")}
              style={{ position: "absolute", top: -8, right: -8, width: 26, height: 26, borderRadius: "50%", background: "#ef4444", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <X size={12} color="#fff" />
            </button>
          </div>
        ) : (
        ...
        <input type="file" name="pdf_file" accept=".pdf" onChange={handleFileChange} id="pdf-upload" style={{ display: "none" }} />
      </div>
    </div>
  );
    ...
    <div style={{ fontFamily: "'DM Sans', 'Segoe UI', sans-serif", background: "#f4f5f7", minHeight: "100vh", padding: "24px 20px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12, marginBottom: 24 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <h1 style={{ fontSize: 20, fontWeight: 700, color: "#111827", margin: 0 }}>Add New Course</h1>
            <p style={{ fontSize: 12, color: "#9ca3af", margin: "2px 0 0" }}>Fill in the details to create a new course</p>
          </div>
        </div>
        ...
        <div className="header-actions" style={{ display: "flex", gap: 10 }}>
          ...
          <button onClick={handleSubmit} disabled={loading}
            style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 20px", border: "none", borderRadius: 10, background: loading ? "#93c5fd" : "#2563eb", color: "#fff", fontSize: 13, fontWeight: 600, cursor: loading ? "not-allowed" : "pointer", minWidth: 130, justifyContent: "center" }}>
            {loading ? (
              <>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={sectionStyle}>
          ...
          <div style={{ padding: 24 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <p style={{ fontSize: 11, color: "#9ca3af", marginTop: 5 }}>Auto-generated from course name. You can customize it.</p>
            </div>
        ...
        <div style={sectionStyle}>
          <div style={sectionHeaderStyle}>
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: 11, color: "#9ca3af" }}>Additional information (optional)</p>
            </div>
          </div>
          <div style={{ padding: 24 }}>
        ...
        <div style={sectionStyle}>
          <div style={sectionHeaderStyle}>
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: 11, color: "#9ca3af" }}>Upload course images and materials (optional)</p>
            </div>
          </div>
          <div style={{ padding: 24 }}>
            <div className="media-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              <UploadBox preview={imagePreview} onRemove={() => removeFile("image")} inputId="image-upload" inputName="image" accept="image/*" label="Course Image" hint="PNG, JPG up to 5MB" />
              <UploadBox preview={bannerPreview} onRemove={() => removeFile("banner_img")} inputId="banner-upload" inputName="banner_img" accept="image/*" label="Banner Image" hint="PNG, JPG up to 5MB" />
              <UploadBox preview={iconPreview} onRemove={() => removeFile("icon")} inputId="icon-upload" inputName="icon" accept="image/*" label="Course Icon" hint="PNG, JPG up to 2MB" isSmall />
              <PdfBox />
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, paddingTop: 4 }}>
          ...
          <button onClick={handleSubmit} disabled={loading}
            style={{ display: "flex", alignItems: "center", gap: 6, padding: "11px 24px", border: "none", borderRadius: 10, background: loading ? "#93c5fd" : "#2563eb", color: "#fff", fontSize: 13, fontWeight: 600, cursor: loading ? "not-allowed" : "pointer", minWidth: 140, justifyContent: "center" }}>
            {loading ? (
              <>
                <div style={{ width: 14, height: 14, border: "2px solid rgba(255,255,255,0.4)", borderTopColor: "#fff", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
                Saving...
