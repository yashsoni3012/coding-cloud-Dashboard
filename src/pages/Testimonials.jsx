...
export default function Testimonials() {
  ...
  const allOnPageSelected =
    paginated.length > 0 && paginated.every((t) => selectedRows.includes(t.id));
  const toggleSelectAll = () => {
    if (allOnPageSelected)
      setSelectedRows((prev) =>
        prev.filter((id) => !paginated.map((t) => t.id).includes(id)),
      );
    else
      setSelectedRows((prev) => [
        ...prev,
        ...paginated.map((t) => t.id).filter((id) => !prev.includes(id)),
      ]);
  };
  const toggleRow = (id) =>
    setSelectedRows((prev) =>
  ...
  const statCards = [
    { label: "Total Reviews", value: testimonials.length, pct: 72 },
    {
      label: "Avg Rating",
      value: `${avgRating}★`,
      pct: (parseFloat(avgRating) / 5) * 100,
    },
    {
              ...
              <thead>
                <tr style={{ borderBottom: "1px solid #f3f4f6" }}>
                  <th style={{ padding: "12px 16px", width: 44 }}>
                    <div
                      className={`cb-t2${allOnPageSelected ? " checked" : ""}`}
                  ...
                  {["Reviewer", "Rating", "Review", "Date", ""].map(
                    (col, i) => (
                      <th
                        key={i}
                        className={i >= 2 && i <= 3 ? "hide-mob-t2" : ""}
                        style={{
                          padding: "12px 14px",
                          textAlign: "left",
                          fontSize: 12,
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: "14px 16px" }}>
                        <div
                          className={`cb-t2${isSelected ? " checked" : ""}`}
                        ...
                        >
                          <div
                            style={{
                              ...
                              height: 38,
                              borderRadius: 10,
                              background: hasAvatar ? "#f3f4f6" : color,
                              display: "flex",
                              alignItems: "center",
                              justifyContent: "center",
                              flexShrink: 0,
                      ...
                      <td
                        className="hide-mob-t2"
                        style={{ padding: "14px 14px", maxWidth: 280 }}
                      >
                        <p
                          style={{
                            margin: 0,
        ...
        {filteredTestimonials.length > 0 && (
          ...
          >
            ...
            <span style={{ fontSize: 13, color: "#6b7280" }}>
              ...
              of {totalPages}&nbsp;·&nbsp;{filteredTestimonials.length}{" "}
              testimonials
            </span>
            <button
              className="page-btn-t2"
