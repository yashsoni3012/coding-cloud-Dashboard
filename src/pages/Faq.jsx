...
export default function FAQs() {
  ...
  const {
    ...
    queryKey: ["faqs"],
    queryFn: fetchFaqs,
    select: (data) => data.map((faq, index) => ({ ...faq, display_id: index + 1 })),
  });
  ...
  const getSortIcon = (key) => {
    if (sortConfig.key !== key) return <SortAsc size={13} style={{ color: "#cbd5e1" }} />;
    return sortConfig.direction === "asc" ? (
      <SortAsc size={13} style={{ color: "#7c3aed" }} />
    ) : (
      <SortDesc size={13} style={{ color: "#7c3aed" }} />
  ...
  const getColor = (id) => avatarColors[(id || 0) % avatarColors.length];
  const getInitials = (question) => (question ? question.slice(0, 2).toUpperCase() : "FA");
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#f8fafc", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ textAlign: "center" }}>
          ...
          <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
          <p style={{ marginTop: 14, color: "#94a3b8", fontSize: 15, fontWeight: 500 }}>Loading FAQs…</p>
        </div>
      </div>
    );
  }
    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc", fontFamily: "'DM Sans', 'Segoe UI', sans-serif" }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "28px 16px" }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 5 }}>
            ...
            <span style={{ padding: "3px 11px", background: "#ede9fe", color: "#6d28d9", fontSize: 13, fontWeight: 700, borderRadius: 99 }}>{faqs.length}</span>
          </div>
          <p style={{ fontSize: 14, color: "#94a3b8", margin: 0, paddingLeft: 48 }}>Manage frequently asked questions across courses</p>
        </div>
        ...
        <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2e8f0", padding: "14px 18px", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
          <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 10 }}>
            ...
            {/* Add FAQ */}
            <button className="faq-add-btn" onClick={() => navigate("/add-faq")}>
        ...
        {/* Table / Empty state */}
        {filteredFaqs.length === 0 ? (
          <div className="faq-animate" style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2e8f0", padding: "64px 24px", textAlign: "center" }}>
            ...
            <h3 style={{ fontSize: 16, fontWeight: 700, color: "#1e293b", margin: "0 0 6px" }}>No FAQs found</h3>
            <p style={{ fontSize: 14.5, color: "#94a3b8", margin: "0 0 20px" }}>
              {searchTerm || filters.course !== "all" ? "Try adjusting your filters or search term." : "Get started by adding your first FAQ."}
            </p>
            {searchTerm || filters.course !== "all" ? (
              <button
                onClick={() => { setSearchTerm(""); setFilters({ course: "all" }); }}
          ...
          <div className="faq-animate" style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2e8f0", overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", minWidth: 700, borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ borderBottom: "2px solid #f1f5f9", background: "#fafafa" }}>
                    <th style={{ padding: "14px 18px", textAlign: "left", width: 56 }}>
                      <button className="faq-th-btn" onClick={() => handleSort("display_id")}>
                        # {getSortIcon("display_id")}
                      </button>
                    </th>
                    <th style={{ padding: "14px 18px", textAlign: "left" }}>
                      <button className="faq-th-btn" onClick={() => handleSort("question")}>
                        Question {getSortIcon("question")}
                      </button>
                    </th>
                    <th style={{ padding: "14px 18px", textAlign: "left" }}>
                      <button className="faq-th-btn" onClick={() => handleSort("course")}>
                        Course {getSortIcon("course")}
                      </button>
                    </th>
                    <th style={{ padding: "14px 18px", textAlign: "right" }}>
                      ...
                      >
                        ...
                        <td style={{ padding: "15px 18px" }}>
                          <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                            <div style={{ width: 32, height: 32, borderRadius: 8, background: color, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 11, fontWeight: 700, flexShrink: 0 }}>
                              {getInitials(faq.question)}
                            </div>
                            <div style={{ flex: 1 }}>
                              <button
                                onClick={(e) => { e.stopPropagation(); toggleFaq(faq.id); }}
                                style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%", background: "none", border: "none", cursor: "pointer", textAlign: "left", padding: 0 }}
                              >
                                <span style={{ fontSize: 15, fontWeight: 600, color: "#1e293b" }}>{faq.question}</span>
                                <ChevronDown size={15} style={{ color: "#94a3b8", transition: "transform 0.2s", transform: isExpanded ? "rotate(180deg)" : "none", flexShrink: 0 }} />
                              </button>
                              {isExpanded && (
                                <div style={{ marginTop: 10, paddingLeft: 10, borderLeft: "2px solid #ede9fe" }}>
                                  <p style={{ fontSize: 14, color: "#64748b", lineHeight: 1.6, margin: 0, whiteSpace: "pre-wrap" }}>{faq.answer}</p>
                                </div>
                              )}
                            </div>
                          </div>
                        ...
                        <td style={{ padding: "15px 18px", verticalAlign: "top" }}>
                          <span style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "4px 10px", background: "#ede9fe", color: "#6d28d9", border: "1px solid #ddd6fe", fontSize: 12.5, fontWeight: 600, borderRadius: 99 }}>
                            <BookOpen size={10} />
                        ...
                        <td style={{ padding: "15px 18px", verticalAlign: "top" }} onClick={(e) => e.stopPropagation()}>
                          <div style={{ display: "flex", justifyContent: "flex-end", gap: 2 }}>
                            <button className="faq-action-btn" onClick={(e) => { e.stopPropagation(); setSelectedFaq(faq); setShowViewModal(true); }} title="View">
                              <Eye size={15} />
                            </button>
                            <button className="faq-action-btn" onClick={(e) => { e.stopPropagation(); navigate(`/edit-faq/${faq.id}`, { state: { faq } }); }} title="Edit">
                              <Edit size={15} />
                            </button>
                            <button className="faq-action-btn" onClick={(e) => handleDeleteClick(e, faq)} title="Delete">
                              <Trash2 size={15} />
                            </button>
                          </div>
                        </td>
            ...
            <div style={{ padding: "13px 18px", background: "#fafafa", borderTop: "1px solid #f1f5f9", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
              <span style={{ fontSize: 13.5, color: "#94a3b8", fontWeight: 500 }}>
                Showing <strong style={{ color: "#475569" }}>{indexOfFirstItem + 1}–{Math.min(indexOfLastItem, filteredFaqs.length)}</strong> of{" "}
                <strong style={{ color: "#475569" }}>{filteredFaqs.length}</strong> FAQs
              </span>
              <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                <button className="faq-page-btn" onClick={() => setCurrentPage((p) => Math.max(p - 1, 1))} disabled={currentPage === 1}>
                  <ChevronLeft size={15} />
                </button>
                {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                  let page = i + 1;
                  ...
                  return (
                    <button key={page} className={`faq-page-btn${currentPage === page ? " active" : ""}`} onClick={() => setCurrentPage(page)}>
                      {page}
                    </button>
                  );
                })}
                <button className="faq-page-btn" onClick={() => setCurrentPage((p) => Math.min(p + 1, totalPages))} disabled={currentPage === totalPages}>
                  <ChevronRight size={15} />
                </button>
              </div>
            </div>
      ...
      {showViewModal && selectedFaq && (
        <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          ...
          <div className="faq-animate" style={{ position: "relative", background: "#fff", borderRadius: 20, boxShadow: "0 20px 60px rgba(0,0,0,0.15)", maxWidth: 520, width: "100%", zIndex: 10, overflow: "hidden", maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 22 }}>
                <div style={{ width: 52, height: 52, borderRadius: 14, background: getColor(selectedFaq.id), display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 18, fontWeight: 700, flexShrink: 0 }}>
                  {getInitials(selectedFaq.question)}
                </div>
                <div>
                  <h2 style={{ fontSize: 20, fontWeight: 700, color: "#0f172a", margin: 0 }}>{selectedFaq.question}</h2>
                  <p style={{ fontSize: 13.5, color: "#94a3b8", margin: "3px 0 0" }}>FAQ #{selectedFaq.display_id}</p>
                </div>
              </div>
            ...
            <div style={{ padding: "14px 24px", background: "#f8fafc", borderTop: "1px solid #f1f5f9", display: "flex", justifyContent: "flex-end", gap: 10, flexShrink: 0 }}>
              <button className="faq-close-btn" onClick={() => setShowViewModal(false)}>Close</button>
              <button
      ...
      {showDeleteModal && faqToDelete && (
        <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          ...
          <div className="faq-animate" style={{ position: "relative", background: "#fff", borderRadius: 20, boxShadow: "0 20px 60px rgba(0,0,0,0.15)", maxWidth: 400, width: "100%", zIndex: 10, padding: 24 }}>
            <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
              ...
              <div style={{ flex: 1 }}>
                <h3 style={{ fontSize: 18, fontWeight: 700, color: "#0f172a", margin: "0 0 4px" }}>Delete FAQ</h3>
                <p style={{ fontSize: 14, color: "#64748b", margin: 0 }}>
                  Are you sure you want to delete "<strong style={{ color: "#1e293b" }}>{faqToDelete.question}</strong>"? This action cannot be undone.
                </p>
              </div>
            </div>

            <div style={{ marginTop: 24, display: "flex", justifyContent: "flex-end", gap: 10 }}>
              <button className="faq-close-btn" onClick={() => setShowDeleteModal(false)} disabled={deleteLoading} style={{ opacity: deleteLoading ? 0.5 : 1 }}>
                Cancel
              </button>
              <button
                onClick={handleDeleteConfirm}
