...
// Helper to get all category names (array)
const getAllCategoryNames = (course) => {
  if (course.categories && Array.isArray(course.categories)) {
    return course.categories.map((cat) => cat.name).filter(Boolean);
  }
  // fallback to category_details (old format)
  if (course.category_details?.name) return [course.category_details.name];
  return [];
};
  ...
  const categories = useMemo(() => {
    ...
    courses.forEach((c) => {
      ...
      if (c.categories && Array.isArray(c.categories)) {
        c.categories.forEach((cat) => catMap.set(cat.id, cat));
      }
    });
    return [...catMap.values()];
  }, [courses]);
  ...
  const filteredCourses = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      result = result.filter(
        (c) =>
          c.name?.toLowerCase().includes(q) ||
          stripHtml(c.text).toLowerCase().includes(q) ||
          getAllCategoryNames(c).some((cat) => cat.toLowerCase().includes(q)),
      );
    }
    if (filters.category !== "all") {
      ...
      result = result.filter((c) => {
        ...
        if (c.categories && Array.isArray(c.categories)) {
          return c.categories.some((cat) => cat.id === categoryId);
        }
        return c.category_details?.id === categoryId;
      });
    }
    if (filters.level !== "all")
      result = result.filter(
    ...
    result.sort((a, b) => {
      ...
      if (sortConfig.key === "id") {
        ...
        aVal = getAllCategoryNames(a)[0]?.toLowerCase() || "";
        bVal = getAllCategoryNames(b)[0]?.toLowerCase() || "";
      } else if (sortConfig.key === "sequence") {
        aVal = a.course_sequence ?? Number.MAX_SAFE_INTEGER;
        bVal = b.course_sequence ?? Number.MAX_SAFE_INTEGER;
      }
        ...
        {filteredCourses.length === 0 ? (
          ...
          <div className="crs-animate">
            ...
            <div className="flex flex-col gap-3 sm:hidden">
              {paginatedCourses.map((course, index) => {
                const plainText = stripHtml(course.text);
                const categoryNames = getAllCategoryNames(course);
                return (
                  <div
                    key={course.id}
                    className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm active:scale-[0.99] transition-transform"
                    ...
                    <tr className="border-b-2 border-slate-100 bg-slate-50/80">
                      ...
                      <th className="px-4 py-3.5 text-left">
                        <button
                          ...
                          className="inline-flex items-center gap-1 text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-slate-600 transition bg-transparent border-0 cursor-pointer p-0"
                        >
                          Categories <SortIcon col="category" />
                        </button>
                      </th>
                      <th className="px-4 py-3.5 text-left">
                        <span className="text-xs font-bold uppercase tracking-widest text-slate-400">
          ...
          <div className="crs-animate relative bg-white w-full sm:max-w-xl md:max-w-2xl sm:rounded-2xl rounded-t-2xl shadow-2xl z-10 overflow-hidden flex flex-col max-h-[92vh]">
            ...
            <div className="relative h-44 sm:h-52 bg-slate-900 flex-shrink-0 overflow-hidden">
              ...
              <div className="absolute bottom-0 left-0 right-0 px-4 sm:px-6 py-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {getAllCategoryNames(selectedCourse).map((cat, idx) => (
                    <span
                      key={idx}
                      className="px-2.5 py-1 bg-white/20 backdrop-blur rounded-full text-xs text-white font-medium"
                    >
                      {cat}
                    </span>
                  ))}
