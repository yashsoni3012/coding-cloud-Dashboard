...
//     setRefreshing(true);
//     try {
//       const topicsRes = await fetch("https://codingcloudapi.codingcloud.co.in/topics/");
//       const topicsJson = await topicsRes.json();
//       if (topicsJson.status === "success") {
//         const parsed = parseTopics(topicsJson.data);
//         setTopicsData(parsed);
...
//         `https://codingcloudapi.codingcloud.co.in/topics/${topicToDelete.id}/`,
//         { method: "DELETE" }
//       );
...
export default function Topics() {
  ...
  useEffect(() => {
    if (rawTopicsData.length) {
      ...
      setTopicsData(parsed);
      setUniqueModules(
        parsed.map((m) => ({ id: m.module_id, name: m.module_name })),
      );
    }
  }, [rawTopicsData]);
                  ...
                  {uniqueModules
                    .filter((m) => {
                      ...
                      const course = courses.find(
                        (c) => c.id === parseInt(filters.course),
                      );
                      return (
                        course &&
                        m.name.toLowerCase().includes(course.name.toLowerCase())
                      );
                    })
                    .map((m) => (
                ...
                <thead>
                  ...
                  >
                    ...
                    <th style={{ padding: "14px 18px", textAlign: "left" }}>
                      ...
                      >
                        Module <SortIcon col="module_name" />
                      </button>
                    </th>
                    <th style={{ padding: "14px 18px", textAlign: "right" }}>
                      <span
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                ...
                {getPageNumbers().map((page) => (
                  <button
                    key={page}
                    className={`top-page-btn${safePage === page ? " active" : ""}`}
